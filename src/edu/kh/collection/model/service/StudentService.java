...
public class StudentService {
	...
	// 앞으로 instanceOf 이용해서 student를 검사할 필요 없음
	...
	public void ex() {
		...
		// 어레이리스트가 객체 아무거나 다 받을 수 있음 
		// 객체 중 최상위 부모는 오브젝트 피피티에서 여러타입의 데이터가 저장 가능하고
		// 객체만 저장할수 잇다고 함
		// * 매개변수타입이 Object인데 이 말은 모든 객체를 매개변수로 전달 할 수 있다는 거
		...
		// 지금 인덱스 0번만 있어서 다른 숫자 입력하면 오류 남
		...
		if( studentList.get(0) instanceof Student) { // 인스턴트 오브로 스튜던트 객체가 맞는지 확인
			System.out.println(((Student)studentList.get(0)).getName()); // 다운캐스팅 해야됨 왜? 겟의 반환형이 오브젝트 오브젝트는 겟네임 없음 그래서 다운캐스팅해야댐
		}
		// 어러객체를 저장할 수 있는 컬렉션의 장점을 얘가 방해하는거 같음
		// -> 길고 복잡함
		// -> 컬렉션의 장점인 여러 객체 저장이 방해 될 정도임
	...
	/**
	 * 메뉴 출력용 메서드 여기에 작성된 내용이 메서드에 설명으로 뜸
	 * @author seungmi0314
	 * alt + shift + j
	 */
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				switch(menuNum) {
				case 1: System.out.println(addStudent());break;  // 반환형이 스트링이라 프린트 구문 안에 실행시켜줘야 함
				...
				sc.nextLine();  // 캐치는 잘못 입력했을때 오는 곳이니까 이걸로 입력버퍼에 남아있는 잘못 입력된 문자열 제거
				menuNum = -1; // 첫 반복 시 잘못 입력하는 경우 menuNum이 0을 가지고 있어 종료 됨
	...
	public void selectAll() {
		...
		// studentList가 비어잇는 경우 "학생 정보가 없습니다" 출력
		
		// List가 비어있는지 확인하는 방법:
		// boolean 자료형에 List.isEmpty() : 비어있으면 True 반환
		
		//if(studentList.size() == 0) {
		if(studentList.isEmpty()) { // studentList가 비어있으면 true반환
			System.out.println("학생 정보가 없습니다");
			
			return; // 반환혀 없는(void) return 구문은 현재 메소드를 종료하고 호출한 곳으로 돌아감
					// 자동으로 else문을 쓴것같은 효과
		}
		/*
		// 일반 for문
		...
		// (순차적 : 0번 인덱스부터 마지막 요서 까지 인덱스를 1씩 증가하면서 접근하는 것)
		
		// [작성법]
		// for(컬렉션 or 배열에서 꺼낸 한개의 요소를 저장 할 변수 : 컬렉션명 또는 배열명) { }
		// 왼쪽으로 오른쪽껄 검사
		int index = 0;
	...
	public String removeStudent() throws InputMismatchException {
		...
		sc.nextLine();
				...
				// 3) 입력된 숫자가 studentList 범위 내 인덱스 번호인가? 
					...
					if(ch == 'Y') {
						// == 이거 차형이라서 가능한거임 String이면 .equals()로 해야됨
